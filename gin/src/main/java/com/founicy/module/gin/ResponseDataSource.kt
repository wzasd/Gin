...
import com.founicy.module.gin.disposables.disposable
import com.founicy.module.gin.executors.ArchTaskExecutor
...
public class ResponseDataSource<T> : DataResponse<T> {
  ...
  public var concatStrategy: DataResponse.ConcatStrategy = DataResponse.ConcatStrategy.CONTINUOUS
  ...
  private fun emitResponseToObserver() {
    if (data != empty && (data as ApiResponse<T>) is ApiResponse.Success<T>) {
      ...
      this.concat?.invoke()
    } else if (concatStrategy == DataResponse.ConcatStrategy.CONTINUOUS) {
      this.concat?.invoke()
    }
  }
   ...
   */
  override fun <R> concat(dataSource: DataResponse<R>): DataResponse<R> {
